...
public class AsnsHandlerTest {
	...
	public void getASTest() {
		SearchResults sr = asns.getAutonomousSystem(PRIVATE_AS_RANGE_START);
	...
	public void addAutonmousSystemTest() {
		...
		SearchResults sr = asns.getAutonomousSystem(testNum);
		while (sr.getCount() != 0) {
			sr = asns.getAutonomousSystem(testNum++);
		}
		as.setAsn(testNum);
		as.setCountry(TEST);
		as.setName(TEST);
		boolean added = asns.addAutonmousSystem(as);
		assertTrue(added);
		SearchResults confirm = asns.getAutonomousSystem(as.getAsn());
